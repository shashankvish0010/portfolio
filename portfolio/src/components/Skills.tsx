...
const Skills: React.FC<CardType> = (props: CardType) => {
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: props.delay }}
            ...
            <span className='h-max md:w-[25vw] w-[65vw] flex flex-col gap-1'>
                <span>
                    <p className='header_list text-sm'>{props.name}</p>
                </span>
                <span className={`bg-indigo-600 h-max p-1 text-xs rounded text-white text-end w-[${props.percent}%]`}>{props.percent}%</span>
            </span>
        </motion.div>
    )
}
